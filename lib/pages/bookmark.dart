        ...
        body: TabBarView(children: <Widget>[
          sb.guestUser == true
              ? EmptyPage(
                  ...
                  message: 'sign in first'.trim(),
                  message1: "sign in to save your favourite places here".trim(),
                )
              : BookmarkedPlaces(),
          sb.guestUser == true
              ? EmptyPage(
                  icon: Feather.user_plus, 
                  message: 'sign in first'.trim(),
                  message1: "sign in to save your favourite blogs here".trim(),
                )
              : BookmarkedBlogs(),
        ]),
      ),
          ...
          if (snapshot.hasData) {
            if (snapshot.data.length == 0)
              return EmptyPage(
                ...
                message: 'no places found'.trim(),
                message1: 'save your favourite places here'.trim(),
              );
            else
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data.length == 0)
              return EmptyPage(
                icon: Feather.bookmark,
                message: 'no blogs found'.trim(),
                message1: 'save your favourite blogs here'.trim(),
              );
